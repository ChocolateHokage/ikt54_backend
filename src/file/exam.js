...
import Students from "../student/student.js";
...
class Exam {
	...
	async create(payload) {
		...
		let subtitle = new docx.TextRun({
			text: "По дисциплине: " + payload.subject[0].full_name, //+ (payload.subject[0].note ? (' (' + payload.subject[0].note + ')') : ""),
			bold: true,
			size: 28,
			break: 2,
		});
			...
			new docx.TableRow({
				children: [
					...
					new docx.TableCell({
						children: [
							new docx.Paragraph({
								children: [new docx.TextRun({ text: "№ билета", size: 24 })],
								alignment: docx.AlignmentType.CENTER,
							}),
						],
						width: {
	...
	async save(callback) {
		let payload = {
			...
			students: await Students.getAllStudents(this.class_id),
		};
		...
		return docx.Packer.toBuffer(this.doc).then((buffer) => {
			...
			// fs.writeFileSync("src/static/files/" + fileName, buffer)
...
// let e = new Exam({ class_id: process.env.CID, subject_id: process.env.SID, teacher_id: process.env.TID })
// e.save((err, result) => {
